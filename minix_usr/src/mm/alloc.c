...
{
  register struct hole *iter;
  phys_clicks buffer[NR_HOLES * 2 + 1], MaxBuffSize, i = 0;
  MaxBuffSize = mm_in.m1_i1 / (sizeof(phys_clicks) * 2);
  for(iter = hole_head; i < MaxBuffSize && iter != NIL_HOLE && iter->h_base < swap_base; iter = iter->h_next) 
  {
	  buffer[i] = iter->h_len;
	  buffer[i+1] = iter->h_base;
    i+=2;
  }
  buffer[i] = 0;
 ...
 */
 ...
 case FIRST:
  ...
  case WORST:
  do {
  	to_del = NIL_HOLE;	/* to_del - best one's previos */
	  best = hp = hole_head;	/* best - best one */
	  while(hp != NIL_HOLE && hp->h_base < swap_base) {
		if(hp->h_len > best->h_len) {
			best = hp;
			to_del = prev_ptr;
		}
		prev_ptr = hp;
	...
	if(best->h_len >= clicks) {
		old_base = best->h_base;
		best->h_base += clicks;
		best->h_len -= clicks;

		if(!best->h_len)
			del_slot(to_del, best);      	
